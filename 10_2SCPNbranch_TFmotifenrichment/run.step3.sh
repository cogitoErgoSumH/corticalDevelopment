...
cat human_gained_lost_enhancer.tmp2.bed | awk 'BEGIN{OFS="\t";}{print $4,$10}' | awk 'BEGIN{sum=0;tmp="chr1-36252856-36253843";}{if($1!=tmp) {print tmp,sum;sum=0;} tmp=$1;sum=sum+$2;}END{print tmp,sum;}' > human_gained_lost_enhancer.tmp3.bed
...
head(dfplot)
# dfplot <- dfplot[-which(rownames(dfplot)=="FOSL1..JUN_MA1128.1"),]
# dfplot <- dfplot[-which(rownames(dfplot)=="FOSL2..JUNB_MA1138.1"),]
