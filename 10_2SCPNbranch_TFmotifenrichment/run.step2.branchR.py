...
f = open('node2branchRMatrx_cell_gene_class.csv',encoding = 'utf-8')
data = read_csv(f)

X_train,y_train = data.iloc[:,2:], data.iloc[:,1]
y_train[~(y_train == "2early")] = "Other"
...
clf_best = GradientBoostingClassifier(n_estimators = 100, learning_rate = 1.0, max_depth = 4, random_state = 0).fit(X_train, y_train)
...
feature_importance_table.to_csv("node2_branchR_featuresImportance.txt", sep='\t')