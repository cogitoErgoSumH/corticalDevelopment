...
############# superficialCPN #############
superficialCPNobj <- subset(URDobj, segment %in% c(11,10,4))

pdf(file="DimPlot.combine_by_Combine_CellType.pdf", height=10,width=10)
DimPlot(superficialCPNobj, reduction = "umap_combine", pt.size=1.2, label=TRUE, label.size = 6, group.by='Combine_CellType') + NoLegend() + scale_color_manual(values=c(cell_colors))
dev.off()
saveRDS(superficialCPNobj, file="superficialCPNobj.rds")

superficialCPNobj@active.ident <- as.factor(superficialCPNobj$Combine_CellType)
cur_markers <- FindAllMarkers(superficialCPNobj, assays='RNA', slot='data', only.pos =TRUE, logfc.threshold =0.25)
saveRDS(cur_markers, file="superficialCPNmarkersGene_CellType.rds")