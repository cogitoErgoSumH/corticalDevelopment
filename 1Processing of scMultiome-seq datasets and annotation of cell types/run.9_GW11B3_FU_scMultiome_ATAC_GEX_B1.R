...
counts_NC8 <- Read10X_h5("/data2/TangLabData/ProcessedData/scMultiome/9_GW11B3_FU_scMultiome_ATAC_GEX_B1/outs/filtered_feature_bc_matrix.h5")
...
pdf('9_GW11B3_FU_scMultiome_ATAC_GEX_B1.dimplot.pdf', h=7, w=8)
p1 <- DimPlot(seurat, reduction = "umap_rna", label = TRUE)
print(p1)
dev.off()

pdf('9_GW11B3_FU_scMultiome_ATAC_GEX_B1.split.dimplot.doublet.pdf', h=7, w=15)
p1 <- DimPlot(seurat, reduction = "umap_rna", label = TRUE, split.by='isDoublet')
print(p1)
dev.off()

pdf('9_GW11B3_FU_scMultiome_ATAC_GEX_B1.dimplot.doublet.pdf', h=7, w=8)
...
seurat <- subset(seurat, idents=c(0), invert=T)
...
seurat_clean@meta.data$nCount_RNA_log2 <- log2(seurat_clean@meta.data$nCount_RNA)
pdf("seurat_clean.nCount_RNA_log2.pdf", width=10, height=10)
FeaturePlot(seurat_clean, features="nCount_RNA_log2",reduction = "umap_rna_clean", pt.size=1.2)+
  scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "Spectral"))) +
  theme(legend.position = "right")
dev.off()

cur_assum_rate = 15 / 100
seurat_clean2 <- run_doubletFinder(seurat_clean, cur_assum_rate)

# saveRDS(seurat_clean2, file="seurat.doubletFinder2.rds")
# seurat_clean2 <- readRDS("seurat.doubletFinder2.rds")
seurat_clean2@meta.data <- seurat_clean2@meta.data[,-18]

pdf('9_GW11B3_FU_scMultiome_ATAC_GEX_B1.split.dimplot.doublet.pdf', h=7, w=15)
p1 <- DimPlot(seurat_clean2, reduction = "umap_rna_clean", label = TRUE, split.by='isDoublet')
print(p1)
dev.off()

pdf('9_GW11B3_FU_scMultiome_ATAC_GEX_B1.dimplot.doublet.pdf', h=7, w=8)
p1 <- DimPlot(seurat_clean2, reduction = "umap_rna_clean", label = TRUE, group.by='isDoublet')
print(p1)
dev.off()

pdf("seurat_clean2.nCount_RNA.doublet.pdf", width=10, height=10)
FeaturePlot(subset(seurat_clean2, subset=isDoublet=="Doublet"), features="nCount_RNA_log2",reduction = "umap_rna_clean", pt.size=1.2)+
  scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "Spectral"))) +
  theme(legend.position = "right")
dev.off()

pdf("seurat_clean2.nCount_RNA.singlet.pdf", width=10, height=10)
FeaturePlot(subset(seurat_clean2, subset=isDoublet=="Singlet"), features="nCount_RNA_log2",reduction = "umap_rna_clean", pt.size=1.2)+
  scale_colour_gradientn(colours = rev(brewer.pal(n = 11, name = "Spectral"))) +
  theme(legend.position = "right")
dev.off()

seurat_clean2$isDoublet[seurat_clean2$seurat_clusters==17] <- "Singlet"
seurat_clean2Singlet <- subset(seurat_clean2, subset=isDoublet=="Singlet")

seurat_clean2Singlet <- FindVariableFeatures(seurat_clean2Singlet, selection.method = "vst", nfeatures = 3000)
seurat_clean2Singlet <- ScaleData(seurat_clean2Singlet,features = row.names(seurat_clean2Singlet))
seurat_clean2Singlet <- RunPCA(
  object = seurat_clean2Singlet,
  assay = "RNA",
  npcs = 50,
  verbose = TRUE)
seurat_clean2Singlet <- RunUMAP(seurat_clean2Singlet, dims = 1:50, reduction.name = "umap_rna_clean2", reduction.key = "UMAPRNAclean2_")
seurat_clean2Singlet <- FindNeighbors(seurat_clean2Singlet, dims = 1:50)
seurat_clean2Singlet <- FindClusters(seurat_clean2Singlet,resolution = 0.8)
table(Idents(seurat_clean2Singlet))

library(RColorBrewer)
library(cowplot)
SpatialColors <- colorRampPalette(colors = rev(x = brewer.pal(n = 11, name = "Spectral")))
num <- length(table(Idents(seurat_clean2Singlet)))+1
jBrewColors <- SpatialColors(num)
jBrewColors <- c(jBrewColors[1:ceiling(num/2)], rev(jBrewColors[(ceiling(num/2)+1):num]))

pdf(file="umap_rna_clean2.noLegend.2.pdf", height=10,width=10)
DimPlot(seurat_clean2Singlet, reduction = "umap_rna_clean2", pt.size=1.2, label=TRUE, cols=jBrewColors, label.size = 6) + NoLegend()
dev.off()
...
for (feature_input_list in c("markers")) {
  for (i in 1:length(get(feature_input_list))){
    feature_input <- get(feature_input_list)[i]
    print(feature_input)
    pdf(file=paste0("umap_rna_clean2.FeaturePlots_",feature_input_list,"_",feature_input,".scale.pdf"), width=10, height=10)
    print(FeaturePlot(seurat_clean2Singlet, slot="scale.data",reduction = "umap_rna_clean2", features = feature_input, pt.size=1.2, order=TRUE, label=TRUE, min.cutoff = "q5", max.cutoff = "q95")+ theme(legend.position = "right"))
    dev.off()
    
    # pdf(file=paste0("umap_rna_clean2.FeaturePlots_",feature_input_list,"_",feature_input,".counts.pdf"), width=10, height=10)
    # print(FeaturePlot(seurat_clean2Singlet, slot="counts",reduction = "umap_rna_clean2", features = feature_input, pt.size=1.2, order=TRUE, label=TRUE, min.cutoff = "q1", max.cutoff = "q99")+ theme(legend.position = "right"))
    # dev.off()
  }
}

# saveRDS(seurat_clean2Singlet, file="seurat.doubletFinder.final.rds")
# seurat_clean2Singlet <- readRDS("seurat.doubletFinder.final.rds")
...
seurat <- FindMultiModalNeighbors(seurat,
                                  reduction.list = list("umap_rna_clean2", "umap_atac"),
                                  dims.list = list(1:ncol(Embeddings(seurat,"umap_rna_clean2")),
