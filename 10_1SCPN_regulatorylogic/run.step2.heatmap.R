...
#########################################################################################################################################################
obj <- readRDS("SCPNobj.rds")
cur_markers <- readRDS("SCPNmarkersGene_CellType.rds")
...
deleteID <- which(matrixPeakCounts_sd<0.8)
...
saveRDS(ht_list, file="SCPN_ht_list.rds")

pdf('SCPN.pseudobulk.Peak_RNA.km20.heatmap.pdf', width=13, height=10)
ht_list
dev.off()

save.image(file="SCPN.pseudobulk.Peak_RNA.km20.heatmap.RData")
# load(file="SCPN.pseudobulk.Peak_RNA.km20.heatmap.RData")

######################
load(file="SCPN.pseudobulk.Peak_RNA.km20.heatmap.RData")
rm(list=setdiff(ls(),c("ht_list","matrixPeakScale","col_ha","selectedCell_info","matrixRNAScale","robust_dist")))
save.image(file="figure5A.heatmap.RData")
######################
...
pdf('SCPN_order.pseudobulk.Peak_RNA.km20.heatmap.pdf', width=13, height=10)
ht_list_order
dev.off()
