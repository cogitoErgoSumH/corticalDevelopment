...
f = open('node1branchRMatrx_cell_gene_class.csv',encoding = 'utf-8')
data = read_csv(f)

X_train,y_train = data.iloc[:,2:], data.iloc[:,1]
y_train[~(y_train == "10early")] = "Other"
...
feature_importance_table.to_csv("node1_branchR_featuresImportance.txt", sep='\t')
