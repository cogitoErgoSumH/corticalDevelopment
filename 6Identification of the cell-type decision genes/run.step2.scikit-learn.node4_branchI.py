...
f = open('node4branchIMatrx_cell_gene_class.csv',encoding = 'utf-8')
data = read_csv(f)

X_train,y_train = data.iloc[:,2:], data.iloc[:,1]
y_train[~(y_train == "6early")] = "Other"
...
clf_best = GradientBoostingClassifier(n_estimators = 75, learning_rate = 1.0, max_depth = 5, random_state = 0).fit(X_train, y_train)
...
feature_importance_table.to_csv("node4_branchI_featuresImportance.txt", sep='\t')
