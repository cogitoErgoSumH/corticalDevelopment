...
f = open('node1branchLMatrx_cell_gene_class.csv',encoding = 'utf-8')
data = read_csv(f)

X_train,y_train = data.iloc[:,2:], data.iloc[:,1]
y_train[~(y_train == "7early")] = "Other"
...
feature_importance_table.to_csv("node1_branchL_featuresImportance.txt", sep='\t')