...
df <- readRDS("markergeneDF.rds")

node2_branchL <- linkINFO[linkINFO$gene %in% df[which(df$type=="up"),"gene"],]$peak
node2_branchR <- linkINFO[linkINFO$gene %in% df[which(df$type=="down"),"gene"],]$peak
...
cat node2_branchL_peak.bed | awk 'BEGIN{OFS="\t";}{print $1,$2,$3,$4,"1",$3-$2+1;}' > human_gained_lost_enhancer.tmp1.bed
cat node2_branchR_peak.bed | awk 'BEGIN{OFS="\t";}{print $1,$2,$3,$4,"0",$3-$2+1;}' >> human_gained_lost_enhancer.tmp1.bed
...
cat human_gained_lost_enhancer.tmp2.bed | awk 'BEGIN{OFS="\t";}{print $4,$10}' | awk 'BEGIN{sum=0;tmp="chr1-12401010-12402181";}{if($1!=tmp) {print tmp,sum;sum=0;} tmp=$1;sum=sum+$2;}END{print tmp,sum;}' > human_gained_lost_enhancer.tmp3.bed
...
dfplot <- df_result[df_result$pvalue<0.01,]
TFinfo <- strsplit(row.names(dfplot),"_")
TFdf <- do.call(rbind.data.frame, TFinfo)
colnames(TFdf) <- c("TF","TFid")
dfplot <- cbind(dfplot,TFdf)
head(dfplot)
dfplot <- dfplot[-which(rownames(dfplot)=="ASCL1_MA1631.1"),]
dfplot <- dfplot[-which(rownames(dfplot)=="CEBPG_MA1636.1"),]
dfplot <- dfplot[-which(rownames(dfplot)=="Ptf1A_MA1620.1"),]
dfplot <- dfplot[-which(rownames(dfplot)=="Ptf1a_MA1618.1"),]


dfplot <- within(dfplot,{TF <- factor(TF,levels=dfplot[order(dfplot$pvalue),]$TF)})
ggplot(dfplot[dfplot$zvalue>0,],aes(x=TF,y=-log10(pvalue))) +
  geom_bar(stat="identity") +
  xlab("") + ylab("-log10(pvalue)") + labs(title = "Motifs found more often in CSMN versus CTPN") +
  background +
...
ggplot(dfplot[dfplot$zvalue<0,],aes(x=TF,y=-log10(pvalue))) +
  geom_bar(stat="identity") +
  xlab("") + ylab("-log10(pvalue)") + labs(title = "Motifs found more often in CTPN versus CSMN") +
  background +
  theme(panel.grid =element_blank(),panel.background=element_rect(fill='transparent', color="#000000"),
        axis.ticks.x = element_blank()) + coord_flip()
...
for (tfclass in df_fisher_test$TFclass) {
  a <- fisher.test(matrix(c(df_fisher_test[which(df_fisher_test$TFclass==tfclass),]$Y,df_fisher_test[which(df_fisher_test$TFclass==tfclass),]$N,37,773),nrow = 2), alternative = "greater")
...
df_fisher_test_result <- data.frame()
for (tfclass in df_fisher_test$TFclass) {
  a <- fisher.test(matrix(c(df_fisher_test[which(df_fisher_test$TFclass==tfclass),]$Y,df_fisher_test[which(df_fisher_test$TFclass==tfclass),]$N,7,803),nrow = 2), alternative = "greater")
  df_fisher_test_result <- rbind(df_fisher_test_result,data.frame(TFclass=tfclass,pvalue=a$p.value))
}
# df_fisher_test_result <- df_fisher_test_result[which(df_fisher_test_result$pvalue<0.05),]
ggplot(df_fisher_test_result,aes(x=TFclass ,y=-log10(pvalue))) +
